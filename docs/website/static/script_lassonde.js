...
function staticLoadPlaces() { //a function called staticLoadPlaces
   return [ //immediately returns and arrat that contains several keys which points to information regarding the "place"
       {
...
function renderPlaces(places) {
    ...
    places.forEach((place) => {//loops though each place once
        ...
        model.setAttribute('lassonde-model', './assets/models/LassondeBuilding.gltf');//creates a new attribute called gltf-model and has the value of the local path to the file scene.gltf
        model.setAttribute('rotation', '0 180 0');//creates a new attribute called rotation, and give it the value of 0.5x, 180y, and 0z
        model.setAttribute('animation-mixer', '');//creates a new attribute called animation-mixer and gives it a valie of blank?
        model.setAttribute('scale', '0.5 0.5 0.5');//creates a new attribute called scale, and gives ith the valies of 0.5x, 0.5y, and 0.5z (which just makes the model half it's original size)
